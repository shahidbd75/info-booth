...
export class DegreeComponent implements OnInit, OnDestroy {
  ...
  onSave() {
    ...
    this.subscription.add(
      this.degreeService.save(requestModel).subscribe(
        () => {
  ...
  onUpdate() {
    ...
    this.subscription.add(
      this.degreeService.update(requestModel).subscribe(
        () => {
          this.router.navigate(['cv/degrees']);
        },
  ...
  loadData() {
    this.activatedRoute.params.subscribe((params: Params) => {
      ...
      if (id) {
        this.subscription.add(
          this.degreeService.getById<CommonResponseModel>(id).subscribe((data: CommonResponseModel) => {
