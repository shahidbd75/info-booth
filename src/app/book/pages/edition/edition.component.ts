...
export class EditionComponent implements OnInit, OnDestroy {
  ...
  onSave() {
    ...
    this.subscription.add(
      this.editionService.save(requestModel).subscribe(
        () => {
  ...
  onUpdate() {
    ...
    this.subscription.add(
      this.editionService.update(requestModel).subscribe(
        () => {
          this.router.navigate(['book/editions']);
        },
  ...
  loadData() {
    this.activatedRoute.params.subscribe((params: Params) => {
      ...
      if (id) {
        this.subscription.add(
          this.editionService.getById<EditionResponseModel>(id).subscribe((data: EditionResponseModel) => {
