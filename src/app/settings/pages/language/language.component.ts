...
export class LanguageComponent implements OnInit, OnDestroy {
  ...
  onSave() {
    ...
    this.subscription.add(
      this.languageService.save(requestModel).subscribe(
        () => {
  ...
  onUpdate() {
    ...
    this.subscription.add(
      this.languageService.update(requestModel).subscribe(
        () => {
          this.router.navigate(['settings/languages']);
        },
  ...
  loadData() {
    this.activatedRoute.params.subscribe((params: Params) => {
      ...
      if (id) {
        this.subscription.add(
          this.languageService.getById<LanguageResponseModel>(id).subscribe((data: LanguageResponseModel) => {
